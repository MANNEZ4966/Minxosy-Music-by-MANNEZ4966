      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `_ ${emoji.ERROR} บอทออกจากห้องแล้ว_`
            )
            .setFooter({ text: ee.footertext, iconURL: ee.footericon }),
        ],
      });
      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(
              ` การเล่นเพลงจบแล้ว \n สนับสนุนบอทได้ที่ [TMN](https://tmn.app.link/?deeplink=ascendmoney%3A%2F%2Fwallet.truemoney.co.th%2Fapp%2F660000000005%3Ftype%3Dconstant%26method%3D%257B%2522tmn_phone%2522%253A%25220644314419%2522%257D)`
            )
            .setFooter({ text: ee.footertext, iconURL: ee.footericon }),
        ],
      });
          ...
          case "playp":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${emoji.ERROR} คุณต้องเข้าห้องเสียงที่มี MOX  **`
                );
    ...
    if (message.channel.id === channel.id) {
      if (message.author.bot) {
        ...
        if (!voiceChannel) {
          ...
          return send(message, `คุณต้องเข้าห้องเสียงที่มี MOX`);
        } else {
